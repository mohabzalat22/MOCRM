...
import { STATUS_COLORS } from '../projects/timeline/constants';
import { TemplateAddTaskForm } from './TemplateAddTaskForm';
import { TemplateTaskItem } from './TemplateTaskItem';

// Assuming we can use the same colors or locally define them
...
const EMPTY_TASKS: ProjectTemplateTask[] = [];
...
export function TemplateTaskList({
    ...
    const visibleTasks = useMemo(() => {
        ...
        const subTasksByParent = tasks.reduce(
            ...
            {} as Record<number, ProjectTemplateTask[]>,
        );

        const result: (ProjectTemplateTask & {
            depth: number;
            hasChildren: boolean;
        })[] = [];

        const addTasks = (taskList: ProjectTemplateTask[], depth: number) => {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            templateTaskService.reorderTasks(
