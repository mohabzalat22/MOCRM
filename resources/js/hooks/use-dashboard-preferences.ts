...
export function useDashboardPreferences(
    initialPreferences: DashboardPreference | null,
    currentRange?: string,
) {
    ...
    const savePreferences = useCallback(
        (newLayout: WidgetConfig[], newDateRange: DateRangeKey) => {
            router.post(
                route('dashboard.preferences.update'),
                {
                    layout: newLayout,
    ...
    useEffect(() => {
        ...
        if (
            debouncedLayout.length > 0 &&
            currentLayoutString !== lastSavedLayout.current
        ) {
